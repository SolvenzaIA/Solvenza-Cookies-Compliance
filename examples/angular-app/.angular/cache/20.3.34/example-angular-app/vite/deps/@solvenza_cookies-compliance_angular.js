...
function createReceipt(policyVersion, choices, source, existingReceipt) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  return {
    schema: 1,
    receiptId: (existingReceipt == null ? void 0 : existingReceipt.receiptId) || generateUUID(),
    policyVersion,
    decidedAt: (existingReceipt == null ? void 0 : existingReceipt.decidedAt) || now,
    updatedAt: now,
    source,
    choices
  };
...
var _MemoryStorageProvider = class _MemoryStorageProvider2 {
  get(key) {
    var _a;
    return (_a = _MemoryStorageProvider2.store.get(key)) != null ? _a : null;
  }
  set(key, value) {
    _MemoryStorageProvider2.store.set(key, value);
  }
  remove(key) {
    _MemoryStorageProvider2.store.delete(key);
  }
  clear() {
    _MemoryStorageProvider2.store.clear();
  }
};
...
var IframeGate = class {
  ...
  static lockIframe(iframe, category, serviceId, options) {
    ...
    const bodyText = "Este contenido está bloqueado hasta que autorices esta finalidad de privacidad.";
    container.innerHTML = `
...
var StateManager = class {
  ...
  hasService(serviceId) {
    ...
    if (((_a = service.consent) == null ? void 0 : _a.mode) === "exempt") return true;
...
var ConsentBanner = class {
  ...
  render(config, handlers) {
    ...
    this.remove();
    injectStyles();
    const bannerConfig = ((_a = config.ui) == null ? void 0 : _a.banner) || {};
    const titleText = bannerConfig.title || "Tu privacidad, bajo tu control";
    const descText = bannerConfig.description || "Usamos tecnologías necesarias para el funcionamiento del sitio. Con tu permiso, también podemos utilizar analítica y marketing.";
    const acceptText = bannerConfig.accept || "Aceptar todas";
    const rejectText = bannerConfig.reject || "Rechazar todas";
    const configureText = bannerConfig.configure || "Configurar";
    const privacyUrl = ((_b = config.policy) == null ? void 0 : _b.privacyUrl) || "/politica-privacidad";
    const cookiesUrl = ((_c = config.policy) == null ? void 0 : _c.cookiesUrl) || "/politica-cookies";
    ...
    `;
    (_d = wrapper.querySelector("#consent-btn-reject")) == null ? void 0 : _d.addEventListener("click", () => {
      handlers.onRejectAll();
      this.remove();
    });
    (_e = wrapper.querySelector("#consent-btn-configure")) == null ? void 0 : _e.addEventListener("click", () => {
      handlers.onConfigure();
    });
    (_f = wrapper.querySelector("#consent-btn-accept")) == null ? void 0 : _f.addEventListener("click", () => {
...
var PreferencesModal = class {
  ...
  render(config, currentChoices, handlers) {
    ...
    const prefConfig = ((_a = config.ui) == null ? void 0 : _a.preferences) || {};
    const modalTitle = prefConfig.title || "Preferencias de privacidad";
    const saveText = prefConfig.save || "Guardar selección";
    const acceptAllText = prefConfig.acceptAll || "Permitir todas";
    const rejectAllText = prefConfig.rejectAll || "Rechazar opcionales";
    ...
    for (const [catId, catConfig] of Object.entries(config.categories)) {
      ...
      const isChecked = isRequired ? true : (_b = categoryChoices[catId]) != null ? _b : false;
        ...
        servicesHtml = `
          ...
          ([srvId, srv]) => `
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.2rem 0;">
                  <span style="font-weight: 500;">${srv.label || srvId}</span>
                  <span style="color: var(--consent-muted); font-size: 0.78rem;">${srv.provider || "Terceros"}</span>
                </div>
              `
        ).join("")}
            </div>
        ...
        <div class="consent-category-header">
          <div style="flex: 1; padding-right: 1rem;">
            <div style="display: flex; align-items: center; gap: 0.4rem;">
              <span class="consent-category-name">${catConfig.label}</span>
            ...
            <p class="consent-category-desc">${catConfig.description}</p>
    ...
    (_c = header.querySelector(".consent-dialog-close")) == null ? void 0 : _c.addEventListener("click", () => {
      handlers.onClose();
      this.close();
    });
    (_d = footer.querySelector("#consent-pref-reject")) == null ? void 0 : _d.addEventListener("click", () => {
      handlers.onRejectAll();
      this.close();
    });
    (_e = footer.querySelector("#consent-pref-accept")) == null ? void 0 : _e.addEventListener("click", () => {
      handlers.onAcceptAll();
      this.close();
    });
    (_f = footer.querySelector("#consent-pref-save")) == null ? void 0 : _f.addEventListener("click", () => {
...
var ConsentEngine = class {
  ...
  init(configInput) {
    return __async(this, null, function* () {
      ...
      this.initPromise = (() => __async(this, null, function* () {
        var _a, _b, _c;
        let config;
        if (typeof configInput === "string") {
          const response = yield fetch(configInput);
          if (!response.ok) {
            throw new Error(
  ...
  saveChoices(choices, source) {
    ...
    if (((_b = config.storage) == null ? void 0 : _b.type) === "memory") {
      ...
      CookieStore.set(cookieName, receiptJson, {
        path: ((_c = config.storage) == null ? void 0 : _c.path) || "/",
        maxAgeDays: (_e = (_d = config.consent) == null ? void 0 : _d.maxAgeDays) != null ? _e : 365,
        sameSite: ((_f = config.storage) == null ? void 0 : _f.sameSite) || "Lax",
        secure: (_g = config.storage) == null ? void 0 : _g.secure
      });
    }
    ...
    if (((_h = config.logging) == null ? void 0 : _h.enabled) && config.logging.endpoint) {
      try {
...
//# sourceMappingURL=@solvenza_cookies-compliance_angular.js.map
